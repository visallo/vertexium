...
public abstract class VertexiumNativeScriptBase implements ExecutableScript {
    ...
    private void applyMarkHiddenMutation(
        ...
        hiddenVisibilities.add(builder.build());
    }
    ...
    private void applyAlterPropertyVisibilityMutation(
        ...
        Property propertyWithOldVisibility = getProperty(
            sourceProperties,
        ...
        if (propertyWithOldVisibility == null) {
            throw new VertexiumElasticsearchException("Could not find property: " + propertyWithOldVisibility);
        }

        Property propertyWithNewVisibility = getProperty(
            sourceProperties,
            alterPropertyVisibility.getKey(),
            alterPropertyVisibility.getName(),
            alterPropertyVisibility.getNewVisibility()
        );
        if (propertyWithNewVisibility != null) {
            removePropertyFromSource(propertyWithNewVisibility.getName(), propertyWithNewVisibility.getVisibility());
            sourceProperties.remove(propertyWithNewVisibility);
        }

        removePropertyFromSource(propertyWithOldVisibility.getName(), propertyWithOldVisibility.getVisibility());
        sourceProperties.remove(propertyWithOldVisibility);

        Property.Builder newProperty = Property.newBuilder(propertyWithOldVisibility)
    ...
    private void applySetPropertyMutation(List<Property> properties, long timestamp, SetPropertyMutation mutation) {
        Property existingProperty = getProperty(properties, mutation.getKey(), mutation.getName(), mutation.getVisibility());
        removeProperty(properties, mutation.getKey(), mutation.getName(), mutation.getVisibility());
        ...
        if (existingProperty != null) {
            for (MetadataEntry metadataEntry : existingProperty.getMetadataList()) {
                metadataEntries.put(metadataEntry.getKey() + metadataEntry.getVisibility(), metadataEntry);
            }
        }
        for (MetadataEntry metadataEntry : mutation.getMetadataList()) {
            metadataEntries.put(metadataEntry.getKey() + metadataEntry.getVisibility(), metadataEntry);
        }
        for (MetadataEntry value : metadataEntries.values()) {
            builder.addMetadata(value);
        }
    ...
    protected void updateSourceWithProperties(List<Property> sourceProperties) {
        ...
        Set<String> propertyHiddenVisibilities = new HashSet<>();
        for (Map.Entry<String, List<Property>> entry : propertiesByNameWithVisibility.entrySet()) {
            ...
            for (Property property : entry.getValue()) {
                ...
                for (PropertyHiddenVisibility propertyHiddenVisibility : property.getHiddenVisibilitiesList()) {
                    propertyHiddenVisibilities.add(propertyHiddenVisibility.getVisibility());
                }
        ...
        updateSourceWithHiddenPropertyVisibilities(propertyHiddenVisibilities);
    }

    private void updateSourceWithHiddenPropertyVisibilities(Set<String> propertyHiddenVisibilities) {
        Set<String> fieldNamesToRemove = getExistingHiddenVisibilityFieldNames();
        for (String propertyHiddenVisibility : propertyHiddenVisibilities) {
            String hiddenPropertyFieldName = PropertyNameUtils.getPropertyNameWithVisibility(FieldNames.HIDDEN_PROPERTY, propertyHiddenVisibility);
            fieldNamesToRemove.remove(hiddenPropertyFieldName);
            source.put(hiddenPropertyFieldName, true);
        }
        for (String fieldNameToRemove : fieldNamesToRemove) {
            source.remove(fieldNameToRemove);
        }
    }

    private Set<String> getExistingHiddenVisibilityFieldNames() {
        Set<String> results = new HashSet<>();
        String prefix = FieldNames.HIDDEN_PROPERTY + "_";
