...
import static org.vertexium.elasticsearch5.DefaultIndexSelectionStrategy.*;
import static org.vertexium.elasticsearch5.Elasticsearch5GraphConfiguration.*;
...
public class ElasticsearchResource extends ExternalResource {
    ...
    private void expandVertexiumPlugin(File vertexiumPluginDir) {
        InputStream zipIn = getClass().getResourceAsStream("/vertexium-elasticsearch5-graph-plugin.zip");
        File pluginZip = new File(vertexiumPluginDir.getParentFile(), "vertexium-elasticsearch5-graph-plugin.zip");
    ...
    public Map createConfig() {
        ...
        configMap.put("", Elasticsearch5Graph.class.getName());
        configMap.put(SEARCH_INDEX_PROP_PREFIX, Elasticsearch5GraphSearchIndex.class.getName());
        configMap.put(CLUSTER_NAME, clusterName);
        configMap.put(ES_LOCATIONS, getLocation());
        configMap.put(CONFIG_INDEX_NAME, ES_INDEX_NAME);
        configMap.put(CONFIG_EXTENDED_DATA_INDEX_NAME_PREFIX, ES_EXTENDED_DATA_INDEX_NAME_PREFIX);
        configMap.put(NUMBER_OF_SHARDS, 1);
        configMap.put(NUMBER_OF_REPLICAS, 0);
// TODO        configMap.put(ERROR_ON_MISSING_VERTEXIUM_PLUGIN, true);
        configMap.put(DefaultIndexSelectionStrategy.CONFIG_SPLIT_EDGES_AND_VERTICES, true);
// TODO        configMap.put(LOG_REQUEST_SIZE_LIMIT, 10000);
        configMap.put(MAX_QUERY_STRING_TERMS, 20);
// TODO        configMap.put(EXCEPTION_HANDLER, TestElasticsearch5ExceptionHandler.class.getName());
        configMap.put(INDEX_REFRESH_INTERVAL, -1);
        configMap.put(CONFIG_METADATA_INDEX_NAME, "vertexium-metadata");
        configMap.put(CONFIG_MUTATION_INDEX_NAME, "vertexium-mutation");
...
//        searchIndex.getConfig().getGraphConfiguration().set(INDEX_EDGES, "false");
//        return true;
//    }
