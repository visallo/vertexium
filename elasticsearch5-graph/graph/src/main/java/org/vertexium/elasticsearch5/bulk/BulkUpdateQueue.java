...
public class BulkUpdateQueue {
    ...
    public CompletableFuture<FlushBatchResult> flushSingleBatch() {
        ...
        future.set(bulkUpdateService.submitBatch(batch)
            .whenComplete((flushBatchResult, throwable) -> {
                ...
                Set<String> indexNames = batch.stream()
                    .flatMap(bulkItem -> Arrays.stream(bulkItem.getIndexNames()))
    ...
    public void addDeleteByQuery(DeleteByQueryRequestBuilder request) {
        AtomicReference<CompletableFuture<FlushBatchResult>> future = new AtomicReference<>();
        future.set(bulkUpdateService.submitDeleteByQuery(request)
            .whenComplete((result, throwable) -> {
                lock.executeInWriteLock(() -> {
                    pendingFutures.remove(future.get());
                });

                for (String indexName : request.request().indices()) {
                    indexService.pushChange(indexName);
                }

                if (throwable != null) {
                    throw new VertexiumException("delete by query failed", throwable);
                }
            }));
        pendingFutures.add(future.get());
    }
