...
public class ElasticsearchSearchQueryBase extends QueryBase {
    ...
    private SearchRequestBuilder buildQuery(EnumSet<ElasticsearchDocumentType> elementType, FetchHints fetchHints, boolean includeAggregations) {
        ...
        String[] indicesToQuery = getIndexSelectionStrategy().getIndicesToQuery(getGraph(), this, elementType);
        if (QUERY_LOGGER.isTraceEnabled()) {
            QUERY_LOGGER.trace("indicesToQuery: %s", Joiner.on(", ").join(indicesToQuery));
        }
        indexService.refresh(indicesToQuery);

        List<String> fields = new ArrayList<>();
        fields.add(FieldNames.OUT_VERTEX_ID);
        fields.add(FieldNames.IN_VERTEX_ID);
        fields.add(FieldNames.EDGE_LABEL);
        fields.add(FieldNames.ELEMENT_ID);
        fields.add(FieldNames.ELEMENT_TYPE);
        fields.add(FieldNames.ELEMENT_VISIBILITY);
        fields.add(FieldNames.EXTENDED_DATA_TABLE_NAME);
        fields.add(FieldNames.EXTENDED_DATA_TABLE_ROW_ID);
        fields.add(FieldNames.HIDDEN_VISIBILITY_DATA);
        fields.add(FieldNames.ADDITIONAL_VISIBILITY);
        if (fetchHints.isIncludeProperties()) {
            fields.add(FieldNames.PROPERTIES_DATA);
        }
        ...
        SearchRequestBuilder searchRequestBuilder = getClient()
            ...
            .storedFields(fields.toArray(new String[0]));
    ...
    protected List<QueryBuilder> getFilters(EnumSet<ElasticsearchDocumentType> elementTypes, FetchHints fetchHints) {
        ...
        QueryBuilder excludeSoftDeletesFilter = QueryBuilders.boolQuery()
            .mustNot(QueryBuilders.existsQuery(FieldNames.SOFT_DELETE_DATA));
        filters.add(excludeSoftDeletesFilter);
        ...
        if (!fetchHints.isIncludeHidden()) {
            String[] hiddenVertexPropertyNames = getPropertyNames(FieldNames.HIDDEN_ELEMENT);
        ...
        if ((elementTypes == null || elementTypes.contains(ElasticsearchDocumentType.EDGE))
            ...
            filters.add(QueryBuilders.termsQuery(FieldNames.EDGE_LABEL, edgeLabelsArray));
        }
        ...
        if (elementTypes == null
            ...
            Collection<String> queryableVisibilities = propertyNameService.getQueryableExtendedDataVisibilities(getParameters().getUser());
            TermsQueryBuilder extendedDataVisibilitiesTerms = QueryBuilders.termsQuery(FieldNames.EXTENDED_DATA_TABLE_PROPERTY_VISIBILITIES, queryableVisibilities);

            if (elementTypes == null
                || elementTypes.contains(ElasticsearchDocumentType.EDGE)
                || elementTypes.contains(ElasticsearchDocumentType.VERTEX)
            ) {
                TermsQueryBuilder extendedDataTerms = QueryBuilders.termsQuery(FieldNames.ELEMENT_TYPE,
        ...
        if (getParameters().getIds() != null) {
            ...
            filters.add(QueryBuilders.termsQuery(FieldNames.ELEMENT_ID, idsArray));
        }

        Collection<String> fields = propertyNameService.getQueryableElementTypeVisibilityPropertyNames(getParameters().getUser());
    ...
    protected void applySortProperty(SearchRequestBuilder q, PropertySortContainer sortContainer, AtomicBoolean sortedById) {
        ...
        if (Element.ID_PROPERTY_NAME.equals(sortContainer.propertyName)) {
            ...
            q.addSort(
                SortBuilders.fieldSort(FieldNames.EXTENDED_DATA_TABLE_NAME)
                    .unmappedType(KEYWORD_UNMAPPED_TYPE)
                    .order(esOrder)
            );
        } else if (ExtendedDataRow.ROW_ID.equals(sortContainer.propertyName)) {
            q.addSort(
                SortBuilders.fieldSort(FieldNames.EXTENDED_DATA_TABLE_ROW_ID)
                    .unmappedType(KEYWORD_UNMAPPED_TYPE)
                    .order(esOrder)
            );
        } else if (ExtendedDataRow.ELEMENT_ID.equals(sortContainer.propertyName)) {
            q.addSort(
                SortBuilders.fieldSort(FieldNames.ELEMENT_ID)
                    .unmappedType(KEYWORD_UNMAPPED_TYPE)
                    .order(esOrder)
            );
        } else if (ExtendedDataRow.ELEMENT_TYPE.equals(sortContainer.propertyName)) {
            q.addSort(
                SortBuilders.fieldSort(FieldNames.ELEMENT_TYPE)
                    .unmappedType(KEYWORD_UNMAPPED_TYPE)
                    .order(esOrder)
            );
        } else {
            ...
            if (!indexService.isPropertyInIndex(sortContainer.propertyName)) {
                return;
            }
            ...
            if (propertyNames.length > 1) {
                List<String> fieldNames = Arrays.stream(propertyNames).map(propertyName ->
                    propertyName + (propertyDefinition.getDataType() == String.class ? FieldNames.EXACT_MATCH_PROPERTY_NAME_SUFFIX : "")
                ).collect(Collectors.toList());
                HashMap<String, Object> scriptParams = new HashMap<>();
                scriptParams.put("fieldNames", fieldNames);
                scriptParams.put("esOrder", esOrder.name());
                scriptParams.put("dataType", propertyDefinition.getDataType().getName());
                Script script = new Script(
                    ScriptType.INLINE,
                    VertexiumScriptConstants.SCRIPT_LANG,
                    VertexiumScriptConstants.ScriptId.SORT.name(),
                    scriptParams
                );
                ...
                if (propertyDefinition.getDataType() == String.class) {
                    sortField += FieldNames.EXACT_MATCH_PROPERTY_NAME_SUFFIX;
                    unmappedType = KEYWORD_UNMAPPED_TYPE;
                }
                q.addSort(
    ...
    private ElasticsearchGraphQueryIterable<VertexiumObject> searchResponseToVertexiumObjectIterable(
        SearchResponse response,
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug(
                "elasticsearch results (total hits: %d)",
                hits.getTotalHits()
            );
        }
        ...
        for (SearchHit hit : hits.getHits()) {
            items.add(Elasticsearch5GraphVertexiumObject.createVertexiumObject(getGraph(), hit, fetchHints, user));
        }

        // TODO instead of passing false here to not evaluate the query string it would be better to support the Lucene query
        return createIterable(response, filterParameters, items, response.getTookInMillis(), hits);
    }
        ...
        for (PropertyDefinition propDef : propertyDefinitions) {
            ...
            for (String propertyName : propertyNames) {
                ...
                if (GeoShape.class.isAssignableFrom(propDef.getDataType())) {
                    filters.add(QueryBuilders.boolQuery().mustNot(QueryBuilders.existsQuery(propertyName + FieldNames.GEO_PROPERTY_NAME_SUFFIX)));
                } else if (isExactMatchPropertyDefinition(propDef)) {
                    filters.add(QueryBuilders.boolQuery().mustNot(QueryBuilders.existsQuery(propertyName + FieldNames.EXACT_MATCH_PROPERTY_NAME_SUFFIX)));
                }
            }
        }
    ...
    private QueryBuilder getFilterForHasExtendedDataFilter(HasExtendedDataFilter has) {
        ...
        if (has.getElementId() != null) {
            boolQuery.must(QueryBuilders.termQuery(FieldNames.ELEMENT_ID, has.getElementId()));
        }
        if (has.getTableName() != null) {
            boolQuery.must(QueryBuilders.termQuery(FieldNames.EXTENDED_DATA_TABLE_NAME, has.getTableName()));
        }
        if (has.getRowId() != null) {
            boolQuery.must(QueryBuilders.termQuery(FieldNames.EXTENDED_DATA_TABLE_ROW_ID, has.getRowId()));
        }
        return boolQuery;
    }
    ...
    protected QueryBuilder getFilterForHasAuthorizationContainer(HasAuthorizationContainer hasAuthorization) {
        ...
        Set<String> hashes = stream(hasAuthorization.getAuthorizations())
            .flatMap(authorization -> propertyNameVisibilitiesStore.getHashesWithAuthorization(authorization, user).stream())
        ...
        for (PropertyDefinition propertyDefinition : graph.getPropertyDefinitions()) {
            ...
            Set<String> matchingPropertyHashes = propertyNameVisibilitiesStore.getHashes(propertyName, user).stream()
                .filter(hashes::contains)
                .collect(Collectors.toSet());
            for (String fieldName : propertyNameService.addHashesToPropertyName(propertyName, matchingPropertyHashes)) {
                filters.add(QueryBuilders.existsQuery(propertyNameService.replaceFieldnameDots(fieldName)));
            }
        }

        List<String> internalFields = Arrays.asList(
            FieldNames.ELEMENT_TYPE,
            FieldNames.HIDDEN_ELEMENT,
            FieldNames.HIDDEN_PROPERTY
        );
        internalFields.forEach(fieldName -> {
            Collection<String> fieldHashes = propertyNameVisibilitiesStore.getHashes(fieldName, user);
            Collection<String> matchingFieldHashes = fieldHashes.stream().filter(hashes::contains).collect(Collectors.toSet());
            for (String fieldNameWithHash : propertyNameService.addHashesToPropertyName(fieldName, matchingFieldHashes)) {
                filters.add(QueryBuilders.existsQuery(fieldNameWithHash));
            }
        });

        if (filters.isEmpty()) {
            throw new VertexiumNoMatchingPropertiesException("auth[" + Joiner.on(", ").join(hasAuthorization.getAuthorizations()) + "]");
        }
    ...
    protected QueryBuilder getFilterForHasPropertyContainer(HasPropertyContainer hasProperty) {
        ...
        for (PropertyDefinition propDef : propertyDefinitions) {
            ...
            for (String propertyName : propertyNames) {
                ...
                if (GeoShape.class.isAssignableFrom(propDef.getDataType())) {
                    filters.add(QueryBuilders.existsQuery(propertyName + FieldNames.GEO_PROPERTY_NAME_SUFFIX));
                } else if (isExactMatchPropertyDefinition(propDef)) {
                    filters.add(QueryBuilders.existsQuery(propertyName + FieldNames.EXACT_MATCH_PROPERTY_NAME_SUFFIX));
                }
            }
        }
    ...
    protected QueryBuilder getFilterForGeoComparePredicate(GeoCompare compare, HasValueContainer has) {
        ...
        for (PropertyDefinition propertyDefinition : propertyDefinitions) {
            ...
            String[] propertyNames = Arrays.stream(getPropertyNames(propertyDefinition.getPropertyName()))
                .map(propertyName -> propertyName + FieldNames.GEO_PROPERTY_NAME_SUFFIX)
                .toArray(String[]::new);
            for (String propertyName : propertyNames) {
    ...
    private QueryBuilder getFilterForProperty(Contains contains, HasValueContainer has, String propertyName, Object value) {
        if (Element.ID_PROPERTY_NAME.equals(propertyName)) {
            ...
            propertyName = FieldNames.EXTENDED_DATA_TABLE_NAME;
        } else if (ExtendedDataRow.ROW_ID.equals(propertyName)) {
            propertyName = FieldNames.EXTENDED_DATA_TABLE_ROW_ID;
        } else if (ExtendedDataRow.ELEMENT_ID.equals(propertyName)) {
            propertyName = FieldNames.ELEMENT_ID;
        } else if (ExtendedDataRow.ELEMENT_TYPE.equals(propertyName)) {
            propertyName = FieldNames.ELEMENT_TYPE;
            if (value instanceof ElementType) {
                value = ElasticsearchDocumentType.getExtendedDataDocumentTypeFromElementType((ElementType) value).getKey();
            }
    ...
    private QueryBuilder getFilterForProperty(Compare compare, HasValueContainer has, String propertyName, Object value) {
        ...
        if (Element.ID_PROPERTY_NAME.equals(propertyName)) {
            ...
            propertyName = FieldNames.ELEMENT_TYPE;
            if (value instanceof String) {
                value = ElementType.parse(value);
            }
            if (value instanceof ElementType) {
                value = ElasticsearchDocumentType.getExtendedDataDocumentTypeFromElementType((ElementType) value).getKey();
            } else {
                throw new VertexiumException("Invalid value to compare against element type expected " + ElementType.class.getName() + " found " + value.getClass().toString());
            }
        ...
        switch (compare) {
            ...
            case RANGE:
                ...
                if (!internalField && (startValue instanceof String || endValue instanceof String)) {
                    propertyName = propertyName + FieldNames.EXACT_MATCH_PROPERTY_NAME_SUFFIX;
                }
    ...
    private Object convertQueryValue(Object value) {
        ...
        if (value instanceof Visibility) {
            return ((Visibility) value).getVisibilityString();
        }
        return value;
    }

    protected String[] getPropertyNames(String propertyName) {
        return propertyNameService.getPropertyNames(propertyName, getParameters().getUser());
    }

    public Elasticsearch5GraphSearchIndex getSearchIndex() {
        return (Elasticsearch5GraphSearchIndex) getGraph().getSearchIndex();
    }
    ...
    private QueryBuilder getAdditionalVisibilitiesFilter() {
        return getGraph().getAdditionalVisibilitiesFilter(getParameters().getUser());
    }
    ...
    protected QueryBuilder createQuery(QueryParameters queryParameters) {
        ...
        if (scoringStrategy != null) {
            if (!(scoringStrategy instanceof ElasticsearchScoringStrategy)) {
                throw new VertexiumException(String.format(
                    "scoring strategies must implement %s to work with Elasticsearch, found: %s",
                    ElasticsearchScoringStrategy.class.getName(),
                    scoringStrategy.getClass().getName()
                ));
            }
    ...
    protected List<AggregationBuilder> getElasticsearchGeohashAggregations(GeohashAggregation agg) {
        ...
        for (String propertyName : getPropertyNames(agg.getFieldName())) {
            String visibilityHash = propertyNameService.getPropertyVisibilityHashFromPropertyName(propertyName);
            ...
            geoHashAgg.field(propertyName + FieldNames.GEO_POINT_PROPERTY_NAME_SUFFIX);
    ...
    protected List<AbstractAggregationBuilder> getElasticsearchPercentilesAggregations(PercentilesAggregation agg) {
        String propertyName = propertyNameService.addVisibilityToPropertyName(agg.getFieldName(), agg.getVisibility());
        String visibilityHash = propertyNameService.getPropertyVisibilityHashFromPropertyName(propertyName);
        String aggName = createAggregationName(agg.getAggregationName(), visibilityHash);
    ...
    protected List<AggregationBuilder> getElasticsearchCardinalityAggregations(CardinalityAggregation agg) {
        ...
        if (Element.ID_PROPERTY_NAME.equals(fieldName)
            ...
            if (Element.ID_PROPERTY_NAME.equals(fieldName) || ExtendedDataRow.ELEMENT_ID.equals(fieldName)) {
                fieldName = FieldNames.ELEMENT_ID;
            } else if (ExtendedDataRow.ELEMENT_TYPE.equals(fieldName)) {
                fieldName = FieldNames.ELEMENT_TYPE;
            }
        ...
        if (Element.ID_PROPERTY_NAME.equals(fieldName)
            ...
            for (String propertyName : getPropertyNames(fieldName)) {
                ...
                if (exactMatchProperty) {
                    propertyNameWithSuffix = propertyName + FieldNames.EXACT_MATCH_PROPERTY_NAME_SUFFIX;
                } else {
                    propertyNameWithSuffix = propertyName;
                }

                String visibilityHash = propertyNameService.getPropertyVisibilityHashFromPropertyName(propertyNameWithSuffix);
        ...
        for (String propertyName : getPropertyNames(agg.getPropertyName())) {
            ...
            if (propertyDataType == Date.class) {
                ...
                Script script = new Script(
                    ScriptType.INLINE,
                    VertexiumScriptConstants.SCRIPT_LANG,
                    VertexiumScriptConstants.ScriptId.CALENDAR_FIELD_AGGREGATION.name(),
                    ImmutableMap.of(
    ...
    public String getAggregationName(String name) {
        return propertyNameService.getAggregationName(name);
    }
