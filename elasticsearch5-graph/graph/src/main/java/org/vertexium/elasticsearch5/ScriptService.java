...
public class ScriptService {
    ...
    public ScriptService(StreamingPropertyValueService streamingPropertyValueService) {
    ...
    public <T extends Element> Mutations mutationToScriptParameter(ElementMutation<T> mutation, long timestamp) {
        ...
        if (mutation.getSoftDeleteData() != null) {
            mutations.addMutations(deleteMutationToMutation(mutation, timestamp));
        }
    ...
    private Mutation propertyToMutation(Property property) {
        ...
        for (Metadata.Entry entry : property.getMetadata().entrySet()) {
            setPropertyMutation.addMetadata(metadataEntryToScriptParameter(entry, property.getTimestamp()));
        }
        return Mutation.newBuilder()
    ...
    private MetadataEntry metadataEntryToScriptParameter(Metadata.Entry entry, long timestamp) {
    ...
    public Value objectToValue(Object obj) {
        ...
        } else if (obj instanceof Visibility) {
            Visibility visibility = (Visibility) obj;
            builder.setVisibilityValue(visibility.getVisibilityString());
    ...
    public Object valueToJavaObject(Value value) {
        switch (value.getValueCase()) {
            ...
            case VISIBILITY_VALUE:
                return new Visibility(value.getVisibilityValue());
            ...
            default:
                throw new VertexiumElasticsearchException("not implemented: " + value.getValueCase());
        }
    }
    ...
    public org.vertexium.Property protobufPropertyToVertexium(org.vertexium.elasticsearch5.models.Property prop, FetchHints fetchHints) {
        ...
        return new MutablePropertyImpl(
            ...
            prop.getName(),
            value,
            protobufMetadataToVertexium(prop.getMetadataList(), prop.getTimestamp(), fetchHints),
            prop.getTimestamp(),
            hiddenVisibilities,
    ...
    public org.vertexium.Metadata protobufMetadataToVertexium(
        ...
        for (MetadataEntry entry : metadata) {
            if (!fetchHints.isIncludePreviousMetadata() && entry.getTimestamp() < propertyTimestamp) {
                continue;
            }
            results.add(
                entry.getKey(),
    ...
    public Metadata metadataListToMetadata(List<MetadataEntry> metadataList) {
        Metadata metadata = Metadata.create();
        for (MetadataEntry metadataEntry : metadataList) {
            metadata.add(
                metadataEntry.getKey(),
                valueToJavaObject(metadataEntry.getValue()),
                new Visibility(metadataEntry.getVisibility())
            );
        }
        return metadata;
    }

    public List<Mutation> edgeMutationToVertexMutations(
        ...
        if (edgeMutation.isDeleteElement() || edgeMutation.getSoftDeleteData() != null) {
            DeleteEdgeToVertexMutation.Builder builder = DeleteEdgeToVertexMutation.newBuilder()
                .setEdgeId(edgeMutation.getId())
                .setDirectionOut(direction == Direction.OUT)
                .setEdgeLabel(edgeMutation.getEdgeLabel())
                .setOtherVertexId(otherVertexId)
                .setEdgeVisibility(edgeMutation.getVisibility().getVisibilityString())
                .setSoftDelete(edgeMutation.getSoftDeleteData() != null);
            if (edgeMutation.getSoftDeleteData() != null) {
                Object eventData = edgeMutation.getSoftDeleteData().getEventData();
                if (eventData != null) {
                    builder.setEventData(objectToValue(eventData));
                }
            }
            DeleteEdgeToVertexMutation deleteEdgeToVertexMutation = builder
                .build();
            Long ts = edgeMutation.getSoftDeleteData() == null ? null : edgeMutation.getSoftDeleteData().getTimestamp();
            if (ts == null) {
                ts = timestamp;
            }
            results.add(Mutation.newBuilder()
                .setTimestamp(ts)
                .setDeleteEdgeToVertexMutation(deleteEdgeToVertexMutation)
                .build());
        } else {
            AddEdgeToVertexMutation addEdgeToVertexMutation = AddEdgeToVertexMutation.newBuilder()
                .setEdgeId(edgeMutation.getId())
                .setDirectionOut(direction == Direction.OUT)
                .setEdgeLabel(edgeMutation.getEdgeLabel())
                .setOtherVertexId(otherVertexId)
            ...
            results.add(Mutation.newBuilder()
                .setTimestamp(timestamp)
                .setAddEdgeToVertexMutation(addEdgeToVertexMutation)
                .build());
        }
