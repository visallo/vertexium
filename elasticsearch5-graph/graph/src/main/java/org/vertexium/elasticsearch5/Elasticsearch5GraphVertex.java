...
public class Elasticsearch5GraphVertex extends Elasticsearch5GraphElement implements Vertex {
    ...
    public static Elasticsearch5GraphVertex createFromMutations(
        ...
        return new Elasticsearch5GraphVertex(
            ...
            id,
            fetchHints,
            properties.stream().filter(p -> canRead(user, p)).collect(Collectors.toList()),
    ...
    private static boolean canRead(User user, Property property) {
        if (!user.canRead(property.getVisibility())) {
            return false;
        }
        for (Visibility hiddenVisibility : property.getHiddenVisibilities()) {
            if (user.canRead(hiddenVisibility)) {
                return false;
            }
        }
        return true;
