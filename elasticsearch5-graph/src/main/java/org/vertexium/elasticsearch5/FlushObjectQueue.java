...
public class FlushObjectQueue {
    ...
    public FlushObjectQueue(Elasticsearch5Graph graph) {
        this.graph = graph;
    }
    ...
    public void flush() {
        ...
        while (itemsToFlush > 0) {
            ...
            try {
                ...
                if (isDocumentMissingException(ex)) {
                    try {
                        graph.handleDocumentMissingException(flushObject, ex);
                    } catch (Exception e) {
        ...
        if (didFlushItem) {
            graph.getClient().admin().indices().prepareRefresh().get(); // TODO we could probably get the indices that changed
        }
    }
    ...
    public void add(
        ...
        queue.add(new FlushObject(elementLocation, extendedDataTableName, rowId, actionRequestBuilder, future));
    }
    ...
    public void add(
        ...
        add(elementLocation, null, null, actionRequestBuilder, future);
    }
    ...
    public static class FlushObject {
        ...
        FlushObject(
            ...
            this(elementLocation, extendedDataTableName, extendedDataRowId, actionRequestBuilder, future, 0, 0);
        }
