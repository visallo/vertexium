...
public abstract class GraphTestBase {
    ...
    @Test
    public void testAsyncExistingElementMutations() {
        CompletableFuture<Void> futureSaveInitial = graph
            .prepareVertex("v1", VISIBILITY_A)
            .saveAsync(AUTHORIZATIONS_A)
            .thenCompose((_ignore) -> {
                Vertex v1 = graph.getVertex("v1", AUTHORIZATIONS_A);
                CompletableFuture<Void> futureSaveExisting = v1.prepareMutation()
                    .addPropertyValue("k1", "name", "Joe", VISIBILITY_A)
                    .saveAsync(AUTHORIZATIONS_A)
                    .thenRun(() -> {
                        Vertex v1AfterSave = graph.getVertex("v1", AUTHORIZATIONS_A);
                        assertEquals("Joe", v1AfterSave.getPropertyValue("k1", "name"));
                    });
                graph.flush();
                return futureSaveExisting;
            });
        graph.flush();
        futureSaveInitial.join();
    }
    ...
    public void testReadStreamingPropertyValues() {
        graph.defineProperty("a").dataType(String.class).textIndexHint(TextIndexHint.FULL_TEXT).define();
        graph.defineProperty("b").dataType(byte[].class).textIndexHint(TextIndexHint.NONE).define();
        graph.defineProperty("c").dataType(String.class).textIndexHint(TextIndexHint.FULL_TEXT).define();
