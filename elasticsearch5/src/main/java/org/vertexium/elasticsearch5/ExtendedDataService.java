...
public class ExtendedDataService {
    ...
    void addElementExtendedData(
        ...
        searchIndex.addActionRequestBuilderForFlush(elementLocation, tableName, rowId, updateRequestBuilder.request());
        ...
        rowsByElementTypeAndId.forEach((elementType, elements) -> {
            elements.forEach((elementId, rows) -> {
                for (ExtendedDataRow row : rows) {
                    ...
                    UpdateRequest request = prepareUpdate(
                        graph,
                        elementLocation,
                        tableName,
                        rowId,
                    ...
                    bulkUpdateService.addUpdate(elementLocation, tableName, rowId, request);
                }
            });
        });
    }
