...
public class Elasticsearch5SearchIndex implements SearchIndex {
    ...
    public Elasticsearch5SearchIndex(Graph graph, GraphConfiguration config) {
        ...
        this.bulkUpdateService = new BulkUpdateService(this, indexService, bulkUpdateServiceConfiguration);
    ...
    @Override
    public void deleteElement(Graph graph, ElementId elementId, User user) {
        deleteExtendedDataForElement(elementId);

        String indexName = indexService.getIndexName(elementId);
    ...
    private void deleteExtendedDataForElement(ElementId elementId) {
        try {
            QueryBuilder filter = QueryBuilders.termQuery(ELEMENT_ID_FIELD_NAME, elementId.getId());
            ...
            for (SearchHit hit : searchResponse.getHits()) {
                ...
                bulkUpdateService.addDelete(
                    elementId,
                    hit.getId(),
                    getClient().prepareDelete(hit.getIndex(), hit.getType(), hit.getId()).request()
                );
            }
        } catch (Exception ex) {
            throw new VertexiumException("Could not delete extended data for element: " + elementId.getId());
        }
    }
    ...
    private UpdateRequestBuilder prepareRemoveFieldsFromDocument(String indexName, String documentId, Collection<String> fields) {
        ...
        return getClient().prepareUpdate()
            ...
            .setRetryOnConflict(config.getRetriesOnConflict());
    }
