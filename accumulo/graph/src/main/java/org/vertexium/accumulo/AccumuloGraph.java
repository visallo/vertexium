...
public class AccumuloGraph extends GraphBaseWithSearchIndex implements Traceable {
    ...
    CompletableFuture<Void> savePropertiesAndAdditionalVisibilities(
    ...
    protected CompletableFuture<Void> addMutations(Element element, CompletableMutation... mutations) {
        return addMutations(VertexiumObjectType.getTypeFromElement(element), mutations);
    }

    protected CompletableFuture<Void> addMutations(VertexiumObjectType objectType, CompletableMutation... mutations) {
        ...
        return CompletableFutureUtils.allOf(Arrays.stream(mutations).map(CompletableMutation::getFuture));
    }
    ...
    public AccumuloEdgeBuilderByVertexId prepareEdge(String edgeId, String outVertexId, String inVertexId, String label, Long timestamp, Visibility visibility) {
        ...
        return new AccumuloEdgeBuilderByVertexId(finalEdgeId, outVertexId, inVertexId, label, visibility, elementMutationBuilder) {
            ...
            public SaveResult<Edge> saveAsync(Authorizations authorizations) {
                ...
                try {
                    return savePreparedEdge(this, timestampLong, null, authorizations);
                } finally {
                    trace.stop();
                }
            }
        ...
        return new EdgeBuilder(finalEdgeId, outVertex, inVertex, label, visibility) {
            ...
            public SaveResult<Edge> saveAsync(Authorizations authorizations) {
                ...
                try {
                    ...
                    return savePreparedEdge(this, timestampLong, addEdgeToVertex, authorizations);
                } finally {
                    trace.stop();
                }
            }
    ...
    private SaveResult<Edge> savePreparedEdge(
        EdgeBuilderBase edgeBuilder,
        long timestamp,
        ...
        futures.add(elementMutationBuilder.saveEdgeBuilder(this, edgeBuilder, timestamp));
        ...
        SaveResult<Edge> result = new SaveResult<>(edge);
    ...
    CompletableFuture<Void> saveExtendedDataMutations(
        Element element,
        ElementType elementType,
        IndexHint indexHint,
        Iterable<ExtendedDataMutation> extendedData,
        ...
        futures.add(elementMutationBuilder.saveExtendedDataMarkers(elementId, elementType, extendedData));
        futures.add(elementMutationBuilder.saveExtendedData(
            this,
            elementId,
            elementType,
            extendedData,
        ...
        if (indexHint != IndexHint.DO_NOT_INDEX) {
            futures.add(getSearchIndex().addElementExtendedData(
                this,
                element,
                extendedData,
                additionalExtendedDataVisibilities,
            ...
            for (ExtendedDataDeleteMutation m : extendedDataDeletes) {
                futures.add(getSearchIndex().deleteExtendedData(
                    this,
                    element,
                    m.getTableName(),
                    m.getRow(),
    ...
    CompletableFuture<Void> alterElementVisibility(AccumuloElement element, Visibility newVisibility, Object data) {
        List<CompletableFuture<Void>> futures = new ArrayList<>();
        String elementRowKey = element.getId();
        ...
        try {
            if (element instanceof Edge) {
                ...
                if (elementMutationBuilder.alterEdgeVertexOutVertex(vertexOutMutation, edge, newVisibility)) {
                    futures.add(addMutations(VertexiumObjectType.VERTEX, vertexOutMutation));
                }
                ...
                if (elementMutationBuilder.alterEdgeVertexInVertex(vertexInMutation, edge, newVisibility)) {
                    futures.add(addMutations(VertexiumObjectType.VERTEX, vertexInMutation));
                }
            }

            CompletableMutation m = new CompletableMutation(elementRowKey);
            if (elementMutationBuilder.alterElementVisibility(m, element, newVisibility, data)) {
                futures.add(addMutations(element, m));
            }
    ...
    public CompletableFuture<Void> alterEdgeLabel(AccumuloEdge edge, String newEdgeLabel) {
        return elementMutationBuilder.alterEdgeLabel(edge, newEdgeLabel);
    }

    CompletableFuture<Void> alterElementPropertyVisibilities(AccumuloElement element, List<AlterPropertyVisibility> alterPropertyVisibilities) {
        ...
        if (!propertyList.isEmpty()) {
            return addMutations(element, m);
        }
        return CompletableFuture.completedFuture(null);
    }

    CompletableFuture<Void> alterPropertyMetadatas(AccumuloElement element, List<SetPropertyMetadata> setPropertyMetadatas) {
