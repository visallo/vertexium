...
public class AccumuloGraph extends GraphBaseWithSearchIndex implements Traceable {
    ...
    void softDeleteProperty(AccumuloElement element, Property property, Object data, Authorizations authorizations) {
        Mutation m = new Mutation(element.getId());
        elementMutationBuilder.addPropertySoftDeleteToMutation(m, property, data);
        addMutations(element, m);
    ...
    public void softDeleteVertex(Vertex vertex, Long timestamp, Object data, Authorizations authorizations) {
        ...
        try {
            ...
            addMutations(VertexiumObjectType.VERTEX, getSoftDeleteRowMutation(vertex.getId(), timestamp, data));

            if (hasEventListeners()) {
                queueEvent(new SoftDeleteVertexEvent(this, vertex, data));
            }
        } finally {
            trace.stop();
        }
    ...
    public void softDeleteEdge(Edge edge, Long timestamp, Object data, Authorizations authorizations) {
        ...
        try {
            ...
            Value value = elementMutationBuilder.softDeleteDataToValue(data);
            ...
            addMutations(VertexiumObjectType.EDGE, getSoftDeleteRowMutation(edge.getId(), timestamp, data));

            if (hasEventListeners()) {
                queueEvent(new SoftDeleteEdgeEvent(this, edge, data));
            }
        } finally {
            trace.stop();
        }
    ...
    private Mutation getSoftDeleteRowMutation(String rowKey, long timestamp, Object data) {
        Mutation m = new Mutation(rowKey);
        m.put(AccumuloElement.CF_SOFT_DELETE, AccumuloElement.CQ_SOFT_DELETE, timestamp, elementMutationBuilder.softDeleteDataToValue(data));
        return m;
    }
    ...
    void alterElementPropertyVisibilities(AccumuloElement element, List<AlterPropertyVisibility> alterPropertyVisibilities) {
        ...
        for (AlterPropertyVisibility apv : alterPropertyVisibilities) {
            ...
            elementMutationBuilder.addPropertySoftDeleteToMutation(m, property, null);
