...
import static org.vertexium.elasticsearch7.Elasticsearch7SearchIndex.ELEMENT_ID_FIELD_NAME;
...
public class ElasticsearchSearchVertexQuery extends ElasticsearchSearchQueryBase implements VertexQuery {
    ...
    private QueryBuilder getDirectionInEdgeFilter() {
        QueryBuilder outVertexIdFilter = QueryBuilders.termQuery(Elasticsearch7SearchIndex.IN_VERTEX_ID_FIELD_NAME, sourceVertex.getId());
        if (otherVertexId != null) {
            QueryBuilder inVertexIdFilter = QueryBuilders.termQuery(Elasticsearch7SearchIndex.OUT_VERTEX_ID_FIELD_NAME, otherVertexId);
            return QueryBuilders.boolQuery()
            ...
            for (String vertexId : ids) {
                filters.add(
                    QueryBuilders.boolQuery()
                        .must(QueryBuilders.termQuery(Elasticsearch7SearchIndex.ELEMENT_TYPE_FIELD_NAME, ElasticsearchDocumentType.VERTEX_EXTENDED_DATA.getKey()))
                        .must(QueryBuilders.termQuery(Elasticsearch7SearchIndex.ELEMENT_ID_FIELD_NAME, vertexId)));
            }
        }
