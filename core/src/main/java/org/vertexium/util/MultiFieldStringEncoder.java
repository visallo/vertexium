...
public class MultiFieldStringEncoder {
    ...
    public String[] decode(String str) {
        ...
        if (expectedPartCount != null && parts.length != expectedPartCount) {
            throw new VertexiumException(String.format("Expected %d parts found %d in string \"%s\" while looking for \"%s\"", expectedPartCount, parts.length, str, separator));
        }
    ...
    private String decodeField(String field) {
        try {
            ...
            throw new VertexiumException("Bad encoder", e);
        }
    }
    ...
    public static String timestampToString(ZonedDateTime timestamp) {
        return String.format("%08x", timestamp.toInstant().toEpochMilli());
    }

    public String encode(String... fields) {
        if (expectedPartCount != null && fields.length != expectedPartCount) {
            throw new VertexiumException(String.format("Expected %d parts found %d", expectedPartCount, fields.length));
        }
